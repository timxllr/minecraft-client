...
public class TabGUI {
    ...
    public void drawGui(int posX, int posY, int width) {
        ...
        switch (mode) {
            ...
            case "ambien new": {
                RenderHelper.drawRect(posX - 1, posY - 1, posX + guiWidth, posY + guiHeight - 13, Client.instance().getAmbienNewDarkGreyColor());
                break;
            }
            case "hero": {
                RenderHelper.drawRect(posX - 1, posY - 1, posX + guiWidth, posY + guiHeight - 13, Client.instance().getHeroGreyColor());
                break;
            }
        }
        ...
        for (int i = 0; i < tabsList.size(); i++) {
            ...
            switch (mode) {
                case "ambien old": {
                    RenderHelper.drawRect(x - 1, yOff - 1, x + guiWidth, y + tabHeight * i + 11, i == selectedTab ? Client.instance().getAmbienOldBlueColor() : 0);
                ...
                case "ambien new": {
                    RenderHelper.drawRect(x - 1, yOff - 1, x + guiWidth, y + tabHeight * i + 11, i == selectedTab ? Client.instance().getAmbienNewBlueColor() : 0);
                    Client.instance().getFontManager().getFont("BigNoodleTitling", 20, Font.PLAIN).drawCenteredString(StringUtils.capitalize((tabsList.get(i)).tabName.toLowerCase()), x + 30, yOff + 1, -1);
                    break;
                }
                case "hero": {
                    RenderHelper.drawRect(x - 1, yOff - 1, x + guiWidth, y + tabHeight * i + 11, i == selectedTab ? Client.instance().getHeroGreenColor() : 0);
                    Client.instance().getFontManager().getFont("Raleway Light", 20, Font.PLAIN).drawStringWithShadow(StringUtils.capitalize((tabsList.get(i)).tabName.toLowerCase()), x, yOff + 1, -1);
                    break;
                }
            }
