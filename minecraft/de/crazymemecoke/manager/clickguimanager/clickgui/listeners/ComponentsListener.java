...
import de.crazymemecoke.manager.clickguimanager.clickgui.components.listeners.ValueListener;
		...
		for (Setting set : Client.main().setMgr().getSettings()) {
			if (set.getParentMod() == m) {
				...
				case BOOLEAN:
					...
					toggleButton.setToggled(m.state());
					toggleButton.addClickListener(new ActionListener() {
						@Override
						public void actionPerformed(ActionEvent e) {
							m.setState(toggleButton.isToggled());
						}
					});
					add(toggleButton);
					break;
