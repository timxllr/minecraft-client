...
public class GuiComboBox implements GuiComponent {
	...
	public void renderCaesium() {
		if (extended) {
			...
			/**
			 * for (String combo : setting.getCombos()) { if
			 * (setting.getMode().equalsIgnoreCase(combo)) {
			 * Panel.fR.drawStringWithShadow(combo, posX + 10, posY + innerHeight,
			 * Panel.color); } else { Panel.fR.drawStringWithShadow(combo, posX + 5, posY +
			 * innerHeight, Panel.fontColor); } innerHeight += (Panel.fR.FONT_HEIGHT + 2);*
			 * }
			 */
