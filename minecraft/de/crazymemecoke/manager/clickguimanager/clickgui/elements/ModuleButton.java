...
public class ModuleButton {
    ...
    public ModuleButton(Module imod, Panel pl) {
        ...
        if (Client.main().setMgr().getSettingsByMod(imod) != null)
            for (Setting s : Client.main().setMgr().getSettingsByMod(imod)) {
                if (s.isCheck()) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (mod.isEnabled()) {
            FontUtil.drawTotalCenteredStringWithShadow(mod.name(), x + width / 2, y + 1 + height / 2, new Color(255, 255, 255).getRGB());
        } else {
